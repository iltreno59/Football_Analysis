...
def recommend_exercises_for_player(
    ...
    Если передан ``db`` (сессия SQLAlchemy), используется она: коммит и закрытие не
    выполняются здесь — вызывающий код управляет транзакцией (например FastAPI + RLS).
    ...
    Args:
        ...
        db: Внешняя сессия БД (опционально)
    ...
    try:
        ...
        if not cluster_analysis:
            return {
                "success": False,
                "error": f"Нет кластерного анализа для игрока {player.player_name}",
            }
        ...
        if not league_id:
            return {
                "success": False,
                "error": f"Лига не определена для игрока {player.player_name}",
            }
        ...
        if season_year:
            season_metrics = season_metrics.filter(
                SeasonMetric.season_start_year == season_year
            )
        ...
        for season_metric in season_metrics:
            ...
            actual_value = (
                float(season_metric.season_metric_value)
                if season_metric.season_metric_value
                else 0
            )
            ...
            z_score = (actual_value - float(benchmark.mean)) / float(
        ...
        for deficit in deficit_metrics:
            ...
            exercises_for_metric = (
                db.query(ExerciseForMetric)
                .filter(ExerciseForMetric.metric_id == metric_id)
                .all()
            )
            ...
            for efm in exercises_for_metric:
                ...
                if not any(
                    ...
                    for e in recommended_exercises
                ):
...
def print_recommendation_report(recommendation):
    ...
    for deficit in recommendation["deficits"]:
        ...
        print(
            f"  {severity_icon} {deficit['metric_name']:30s} | Z-score: {deficit['z_score']:6.2f} | Actual: {deficit['actual']:8.2f} | Bench: {deficit['mean']:8.2f}"
        )
    ...
    for exercise in recommendation["exercises"]:
        print(
            f"  • {exercise['exercise_name']:40s} | Метрик: {exercise['metric_name']:20s} | Сложность: {exercise['difficulty']}"
        )
